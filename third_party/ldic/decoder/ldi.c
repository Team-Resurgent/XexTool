        ...
        PFNSEEK         pfnseek)            /* seek in file callback */
{
    ...
    pConfig = (PFLZXDECOMPRESS)pvConfiguration;       /* get a pointer we can use */
    ...
    *pmdhHandle = (LDI_CONTEXT_HANDLE) 0;   /* wait until it's valid */
    ...
    context = (PMDC_CONTEXT)pfnma(sizeof(struct LDI_CONTEXT));
	...
	context->decoder_context = (t_decoder_context*)pfnma(sizeof(t_decoder_context));
...
int LZX_DecodeInsertDictionary(t_decoder_context* ctx, BYTE* dictData, long dictSize)
{
	long ctx_size = ctx->dec_window_size;
	BYTE* ctx_ptr = (BYTE*)ctx->dec_mem_window;
	
	// make sure dictionary data fits into context
	if(dictSize > ctx_size)
		return (MDI_ERROR_BUFFER_OVERFLOW);
	
	// copy dictData into end of context context
	memcpy(ctx_ptr + ctx_size - dictSize, dictData, dictSize);
	
	// fill rest of context with zeros if dictData doesnt fill it
	if(dictSize < ctx_size)
	{
...
// added in for xbox360 kernel usage
// returns:	0 = success
int FAR DIAMONDAPI LDISetWindowData(
		LDI_CONTEXT_HANDLE  hmd,            /* decompression context */
		BYTE* data,                         /* data */
		long size)                          /* size */
{
	PMDC_CONTEXT context = PMDCfromHMD(hmd);             /* get pointer from handle */ 
	if (context->signature != LDI_SIGNATURE)
	{
		return(MDI_ERROR_BAD_PARAMETERS);   /* missing signature */
	}
	if( LZX_DecodeInsertDictionary(context->decoder_context, data, size) != 0)
		return MDI_ERROR_FAILED;
	// success
	return (MDI_ERROR_NO_ERROR);
}
